...
\begin{document}
\input{header}
\noindent\textbf{Assignment 3, Due Tuesday Feb 20}
\vskip.15in
...
\section{A bit of programming - spend 10 minutes on this before Friday
  the 16th}

Start from the Google Colab notebook we developed in class

\url{https://colab.research.google.com/drive/14KRuTQ0W3lphr3ycC7qjFoi5m-CB0Bci#scrollTo=Cl0SPSuivAOv}

make a new cell, and define your own function called ``airy2''. That
function should be the second non-trivial (i.e. not always zero)
...
demonstrated convergence, $|x|\le1$. The end goal is to do this in a
Python notebook. That may be too much programming, though. If the
Python is a barrier, you can check your answers with
Wolfram Alpha (e.g. go to wolframalpha.com and type 
``plot 0.5*(3x\verb|^|2-1) from -1 to
1'' in the box), or via Desmos. Then we can do the Python work in class.!\\
